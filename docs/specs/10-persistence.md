...
**Status:** Implemented (Phase 1 + 2 + 4 + v0.6.1 two-variant atomic-pair amendment) · **Last updated:** 2026-05-18 · **Depends on:** 00 · **Used by:** 02, 03, 04, 05, 06, 07, 08, 09 · **Canonical for:** every JSON schema in this app (`album.json`, `state.json`, `settings.json`) + the atomic-pair invariant for multi-file transactions
...
**Load-time scan trigger and scope.** `AlbumStore.rescan()` runs the atomic-pair scan on every album that has a `reports/` subdirectory, via `scan_reports_dir(reports_dir, *, sanitised_name=sanitise_title(album.name) or "album")` (see `src/album_builder/persistence/atomic_pair.py`). The scan enumerates **both variants** (full and artist-view) for every distinct date stem and processes each pair independently. Illustrative pseudo-code (the real entry point takes `reports_dir` + `sanitised_name` as caller-supplied arguments, not attributes of `Album`):

```
def scan_reports_dir(reports_dir, *, sanitised_name):
    for stem in distinct_date_stems(reports_dir, sanitised_name):
        for suffix in ("", " - artist"):        # full variant first, then artist-view
            html_final = reports_dir / f"{sanitised_name} - {stem}{suffix}.html"
            pdf_final  = reports_dir / f"{sanitised_name} - {stem}{suffix}.pdf"
            html_tmp_pattern = f"{sanitised_name} - {stem}{suffix}.html.*.tmp"
            pdf_tmp_pattern  = f"{sanitised_name} - {stem}{suffix}.pdf.*.tmp"

            has_html = html_final.exists()
            has_pdf  = pdf_final.exists()
            has_html_tmp = any(reports_dir.glob(html_tmp_pattern))
            has_pdf_tmp  = any(reports_dir.glob(pdf_tmp_pattern))

            if has_html != has_pdf:                 # exactly one final exists
                # delete the renamed final AND any orphan tmp from the unrenamed half
                unlink_if_exists(html_final, pdf_final)
...
| `target_count` | int | 1–99 on UI input + domain validation; load-time self-heal may bump above 99 to satisfy `target_count >= len(track_paths)` when an external edit pushes `track_paths` longer (Spec 08 §Symlink filenames depends on this). |
