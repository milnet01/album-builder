...
| Album name begins with `<` (e.g. user names album `"<Demo>"` or `"<b>Loud</b>"`) | Qt's `QToolTip` auto-detects rich text on leading `<`. The tooltip builder uses `Qt::convertFromPlainText` (or equivalent) on each name so the album displays as plain text, not interpreted HTML. |
