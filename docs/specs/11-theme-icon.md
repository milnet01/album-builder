...
**Status:** Implemented (Phase 1 + 2 + 4) · **Last updated:** 2026-05-18 · **Depends on:** 00 · **Used by:** 03, 04, 05, 06, 07, 09, 12 (every UI-bearing spec)

## Purpose

Define the professional visual language of the app — `dark-colourful` is the default palette and the reference for the design system; Spec 19 adds four sibling themes (Light, Dark Ocean, Dark Ember, Dark Slate) through the same `qt_stylesheet(palette)` mechanism, selectable live via `View -> Theme` — and the licensing + sourcing of the application icon.
...
| `x` | `U+0078` (ASCII letter — the source character; visually rendered as multiplication-like via QSS font-size + close-button context) | Toast close affordance | 12 px, `text-secondary`; hover → `text-primary` |
...
| Condition | Behavior |
|---|---|
| User runs Plasma with a light-theme system | Album Builder does not auto-follow the OS/Plasma theme; it uses its own theme (default `dark-colourful`, or whatever the user picked in `View -> Theme`). The per-app override is intentional (OS-follow is out of scope, Spec 19). |
| Plasma's "follow system theme" setting | Ignored — Album Builder applies its own selected theme (Spec 19); OS-theme-following is out of scope. |
| Icon not found at runtime (broken install) | Fallback to a Qt-shipped generic icon; window still opens. |
| User wants a different colour scheme | Pick one of the five built-in themes via `View -> Theme` (Spec 19). User-authored palette JSON in `~/.config/album-builder/themes/` remains a roadmap item (user-customisation is still out of scope). |

## Test contract

Each clause is a testable assertion. Tests must reference its TC ID via a `# Spec: TC-11-NN` marker.

**Phase status — TC-11-01 + TC-11-02 + TC-11-04 shipped in Phase 1 (palette + stylesheet + placeholder contrast — see `tests/ui/test_theme.py`).** TC-11-05..09 shipped across Phase 2 / 3 / 4 as the UI surfaces they reference landed. TC-11-10/11 shipped in Phase 4 (report integration; mirrors `tests/services/test_TC_09_report.py`). TC-11-03 is satisfied implicitly by `tests/ui/test_main_window.py` (the QSS applies cleanly on construction; no stderr parse warnings are emitted in CI). The Phase-1 tests in `tests/ui/test_theme.py` were anchored with `# Spec: TC-11-01/02/04` markers on 2026-06-10.
...
- ~~Light theme.~~ **Delivered by Spec 19** (Light + three dark variants, live switcher).
- High-contrast / accessibility theme (a dedicated one; the Spec 19 themes are WCAG-AA but not a distinct high-contrast mode).
- User-customisable themes (user-authored palettes).
- OS/system-theme following (auto light/dark from the desktop).
