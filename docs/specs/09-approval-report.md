...
6. The `reports/` folder is opened in the file manager via `QDesktopServices.openUrl` (cross-platform: `xdg-open` on Linux, `ShellExecute` on Windows, `open` on macOS - Spec 22 INV-22-4), gated on `settings.ui.open_report_folder_on_approve` (Spec 10 §`settings.json` schema; default `true`).
...
- **TC-09-18** — `QDesktopServices.openUrl(QUrl.fromLocalFile(<reports_folder>))` is invoked exactly once after a successful approve, gated on `settings.ui.open_report_folder_on_approve`. Setting `false` suppresses the call; `true` (default) invokes it. An `openUrl` returning `False` (no handler) or raising is logged and silently ignored — the approve is still successful (Spec 22 INV-22-4).
