...
```
/mnt/Games/Scripts/Linux/Album_Builder/
...
| [21](./21-replaygain.md) | ReplayGain | Opt-in loudness normalization: read ReplayGain tags (ID3 `TXXX`, or the container's own), scale the output volume by a track/album-gain factor (user volume unchanged), toggled via a Playback menu; Phase F of the music-player epic (implemented) |
