...
- **Same source path twice in `track_paths`:** Spec 04 forbids duplicate selection within a single album, so this case is unreachable from the UI; if it occurs via hand-edit of `album.json`, nothing rejects it: the loader keeps both entries as written (only `Album.select` refuses a duplicate). The export pipeline assumes uniqueness and does not defend against it.
...
| Album folder is on a filesystem that doesn't support symlinks (e.g. FAT/exFAT, Windows without Developer Mode) | Skip the numbered entries and emit `playlist.m3u8` (the remembered-path list) only, plus the album's other artifacts; one warn-log, no hardlink, copy, consent dialog, or capability cache (Spec 22 §Purpose supersedes the earlier deferred copy-with-consent design). Drift detection expects **zero** symlinks there (Spec 22 INV-22-3). |
...
- **TC-08-10a/10b** (non-symlink filesystem) — resolved by Spec 22's playlist-only export; the contracts are covered end-to-end by Spec 22 TC-22-02/03 (non-symlink case forced via monkeypatch, since the CI filesystem supports symlinks).
...
- **TC-08-10a** — Filesystem without symlink support (playlist-only, Spec 22 INV-22-1) → `regenerate_album_exports` writes `playlist.m3u8` + the album artifacts, creates **zero** numbered entries, and appends exactly one warn-log (not one-per-track). No hardlink, copy, consent dialog, or `fs-caps.json` cache. (Verified end-to-end by Spec 22 TC-22-02.)
- **TC-08-10b** — Playlist-only drift: on a filesystem without symlink support the drift invariant expects **zero** symlinks, so a playlist-only album reads fresh (no perpetual `needs_regen`), Spec 22 INV-22-3. (Verified by Spec 22 TC-22-03.)
