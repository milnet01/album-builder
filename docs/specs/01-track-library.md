...
| Extension | Tags read | Tag family the container actually uses |
|---|---|---|
| `.mp3`, `.mpeg` | **yes** | ID3v2 |
| `.wav`, `.aiff`, `.aif` | no | ID3v2 chunk (readable, not read) |
| `.flac`, `.ogg`, `.oga`, `.opus` | no | Vorbis comments |
| `.m4a` | no | MP4 atoms |
| `.wma` | no | ASF attributes |
...
new extensions therefore behave exactly like the five non-MP3 extensions that already
...
running it). Reading the other containers' native tag families is a **separate defect**,
filed as MUSI-0359; see the end of this document.
