...
- **Fully-featured music player mode (library-wide playback engine).** (User request 2026-06-17.) Expand Album Builder beyond curation into a general-purpose player: play the whole library (not just an album preview), with a persistent play queue, shuffle/repeat, gapless playback, saved playlists, and a full transport. **Reuse-before-rewrite:** build on the existing `Player` (QMediaPlayer wrapper), `LyricsTracker`, `TransportBar`, and `Library` services rather than a parallel pipeline. Large multi-phase epic; each phase is spec-first (author the spec + run `/cold-eyes` to clean before implementation, per spec discipline). Indicative phases:
  - **Phase A — Playback queue (domain).** An ordered, mutable queue decoupled from `Album`; shuffle + repeat-one/repeat-all modes; next/prev semantics. New pure-Python domain object (frozen track refs + mutable order), no Qt. Needs a new spec (e.g. Spec 14 — playback queue) with TC contracts.
  - **Phase B — Library-wide playback wiring (services + ui).** Play/enqueue any track from `LibraryPane` ("play all", "play from here"); queue survives track changes. Extend `Player` to advance through the queue on track-end (it currently plays one preview source).
  - **Phase C — Transport enrichment (ui).** Shuffle, repeat, next/prev, and a seek/scrubber surfaced in `TransportBar`; gapless transition needs a spike — QtMultimedia/QMediaPlayer gapless is historically limited, so verify the current PyQt6 idiom (pre-roll next source vs. `QMediaPlayer` playlist successor) before committing.
  - **Phase D — Saved playlists (persistence).** Named, reorderable playlists persisted through the atomic-write layer (Spec 10 amendment or new spec): JSON-on-disk, debounced writer, a schema-migration entry, plus playlist CRUD UI.
  - **Phase E — Dedicated player-mode surface (ui).** Folds in the deferred "Player mode (listening UI)" bullet above: a focused/detachable now-playing window (transport + scrubber + synced lyrics, optional skin/visualiser), toggled from `MainWindow`. Reuse `NowPlayingPane` / `LyricsPanel`.
  - **Phase F (research) — Equalizer / audio effects.** QtMultimedia has no native EQ; would need an audio-filter graph or a different audio backend. Spike + gauge demand before committing.
  Sequencing: deferred until the core curation loop is feature-complete (per the existing Player-mode deferral). A->B->C deliver a usable player; D/E/F are follow-on. Each phase gets its own spec + TC contracts before any code lands.
