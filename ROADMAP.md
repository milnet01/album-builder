...
## Audio format support - widen the library scan (Spec 01 amendment) - 2026-08-24

The library scan accepted seven extensions (.mp3, .mpeg, .m4a, .flac, .ogg,
.opus, .wav). This widens it to the mainstream formats a normal music folder
...
- ✅ [MUSI-0358] **Widen SUPPORTED_EXTENSIONS to include .aac, .aiff, .aif, .oga and .wma.**
  ...
  list and move with it.

  Verified before speccing, with real files generated by ffmpeg and read
  back through the installed stack:
    .aac  - decodes (codec aac); mutagen reads duration but CANNOT carry
            tags at all (AACError: doesn't support tags), so every .aac
            track falls back to the file-stem title per Spec 08 Errors.
            Included deliberately with that limitation documented.
    .aiff - decodes (pcm_s16be); ID3 works once a tag block is added.
    .aif  - same format as .aiff, second extension in common use.
    .oga  - decodes (vorbis); tags read natively.
    .wma  - decodes (wmav2); tags read natively via ASF.
  ...
  **Layman:** The app now finds more kinds of music files in your Tracks folder, not just the seven it knew before.
  ...
  Lanes: domain, docs, tests.

- ✅ [MUSI-0359] **Track.from_path reads tags only via ID3, so 9 of 12 supported formats show no metadata.**
  Surfaced by the review-contract gate on MUSI-0358 and verified by running
  the shipped code against tagged files generated by ffmpeg.

  `domain/track.py::from_path` obtains `duration` from the generic
  `mutagen.File(path)` reader but every other field from
  `mutagen.id3.ID3(path)` - `_text`, `_comment_text`, `_lyrics_text`,
  `_first_apic_image` and `_read_replaygain` are all ID3-keyed. A container
  with no ID3 block therefore returns nothing to read.

  Measured: of the twelve extensions the scan accepts, only `.mp3` and
  `.mpeg` yield tags. `.flac`, `.ogg`, `.oga` and `.opus` (Vorbis comments),
  `.m4a` (MP4 atoms), `.wma` (ASF), and `.wav`, `.aiff`, `.aif` (ID3 chunk,
  readable but not read) all take the TC-01-05 placeholder path. Four of
  those shipped long before the format amendment, so this is pre-existing
  rather than introduced by it.

  `.aac` is the one genuine exception and is not covered by this item: ADTS
  has no tag block at all.

  Fix is a per-container key mapping onto the same `Track` fields. Spec 01
  Section Inputs and TC-01-04 would need to move with it, and Spec 08's
  symlink-title rule plus the report and MPRIS metadata all bind to the
  result.
  Resolved (2026-09-21): Track.from_path now prefers ID3 and otherwise
  reads the container's native text tags through a per-container name
  map - Vorbis comments, MP4 atoms, ASF attributes. .wav and .aiff route
  to the ID3 reader via mutagen.File, which reaches the ID3 chunk that
  ID3(path) does not. Reproduced first against ffmpeg-tagged files of
  every affected format, all showing 'Unknown artist'; all now read
  title, artist, album_artist, album, composer. Cover art and ReplayGain
  deliberately did not move - they need per-container decoding rather
  than a name, and are MUSI-0362 and MUSI-0363. Spec 01's per-format
  table and prose amended in the same commit; TC-01-17 added.
  **Layman:** FLAC, M4A, OGG, Opus, WAV and WMA files show "Unknown artist" even when they are fully tagged - the app only reads tags from MP3s.
  Kind: fix.
  Source: review-contract-2026-08-24 spec-01 gate.
  Lanes: domain.

- ✅ [MUSI-0360] **Untagged files report duration_seconds 0.0 because a mutagen FileType with no tags is falsy.**
  Surfaced by the review-contract gate on MUSI-0358 while verifying a
  separate claim; confirmed by running it.

  `domain/track.py::from_path` computes:

        duration = float(mf.info.length) if mf and mf.info else 0.0

  `mf` is a mutagen `FileType`, which is dict-like over its tags. A file
  carrying ZERO tags is therefore falsy, the guard short-circuits, and the
  correct `mf.info.length` is discarded in favour of 0.0.

  Measured on 2-second files: `.wav` and `.aiff` written without tags both
  reported `bool(mf) == False`, `len(mf) == 0` and `info.length == 2.0`,
  and `Track.from_path` returned `duration_seconds == 0.0`. This is not
  format-specific - an untagged `.mp3` hits it too.

  Blast radius: `#EXTINF` in exported M3U playlists (Spec 08 Outputs, which
  specifies integer seconds), duration display in the library pane, and
  SortKey.DURATION ordering.

  The guard should test `mf is not None`, not `mf`. One line, but it needs a
  regression test that pins an untagged file's duration.
  Resolved (2026-09-21): fixed alongside MUSI-0359, whose fix touched
  the same expression. Root cause found by running it: a mutagen
  FileType is dict-like, so bool() counts TAGS - `if mf and mf.info` was
  therefore False for any file carrying no tags, and the duration was
  discarded however long the stream was. Now tests `mf is not None and
  mf.info is not None`. A tagless .wav that reported 0.00 reports 1.00.
  TC-01-18 added and locks the truthiness trap by name.
  **Layman:** A music file with no tags shows a length of zero, which also makes exported playlists claim every such track is 0 seconds long.
  Kind: fix.
  Source: review-contract-2026-08-24 spec-01 gate.
  Lanes: domain.
...
- ✅ [MUSI-0362] **Cover art is read only from ID3 APIC, so non-MP3 containers show no artwork.**
  ...
  `METADATA_BLOCK_PICTURE`, MP4 a `covr` atom, ASF `WM/Picture`.
