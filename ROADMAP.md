...
(`Palette.dark_colourful()` hardcoded); Spec 19 adds a Light theme plus three dark
variants and a live in-app picker.

**Spec-first:** Spec 19 authored + `/cold-eyes` clean (4 loops, 2-3 reviewers/loop —
caught the Spec 11 amendment gaps, a persistence→ui layer inversion, wrong
`accent_warm`/`text_tertiary` WCAG thresholds, and a `write_ui`-in-a-slot qFatal crash
risk, all before code).

**Shipped:**
- `ui/theme.py` — four new `Palette` factories (`light`, `dark_ocean`, `dark_ember`,
  `dark_slate`), each WCAG 2.2 AA (verified by the TC-19-04 contrast test); a `THEMES`
  ordered registry + `palette_for(id)` with degrade-to-`dark-colourful` fallback.
- `persistence/settings.py` — `ALLOWED_THEMES` expanded to the five ids (literal, kept
  Qt/ui-free; TC-19-05 asserts sync with `THEMES`).
- `ui/lyrics_panel.py` + `ui/library_pane.py` — `set_palette()` refresh for the two
  imperative-styled widgets (per-line lyric colours; usage-badge fill).
- `ui/main_window.py` — a menu bar (File / **View → Theme** radio group / Help), a single
  `_apply_theme(id, persist=)` path shared by startup + live switch (re-applies the
  stylesheet, refreshes the imperative widgets, checks the menu, persists `ui.theme`
  with an `OSError` guard so a failed write can't qFatal the app). The hardcoded
  dark-colourful stylesheet is gone; startup applies the persisted theme.
- **Tests:** 35 new (`test_TC_19_themes.py` registry/contrast; `test_TC_19_theme_switch.py`
  menu/apply/startup/idempotence/wrappers/crash-safety). Full suite **756 passed / 12
  skipped**, ruff clean, local-CI green. Verified end-to-end (headless launch: menu
  present, live switch recolours).
- **Cross-spec amendments:** Spec 10 (`ui.theme` now accepts five ids), Spec 11
  (retired "dark-only in v1" / "Light theme out of scope"), Spec 00 index (rows 18 + 19).
- **Known limitation (documented):** `dark-colourful`'s de-emphasized `text_tertiary`
  is ~3.5:1 (below AA) — a pre-existing value, grandfathered; retuning the established
  palette is out of scope here (the four new themes are fully AA).
...
**Test count:** 549 → 562 passing (+13: 5 atomic-pair + 4 artist-view render + 1 catch-all + 3 from earlier in the cycle). Ruff clean. No new third-party deps. ROADMAP fully `✅`-flipped — every status marker closed (except the standing `🔭 Methodology gaps` and `🔭 Future / deferred` queues, which track ongoing-practice + future-work items by design).
...
### 🧪 Test Audit 2026-05-18 (second sweep, post-v0.6.1 debt-sweep)
...
- Pre-pass `datetime.now` ×4 in `tests/domain/test_album.py:241/252/263/264` — all are constructor argument values, never asserted on. Pre-pass regex cannot distinguish "asserted-on timestamp" from "passed to constructor".
- Pre-pass `datetime.now` ×1 in `tests/ui/test_TC_13_library_pane_usage_column.py:414` — same shape (constructor argument).
- Chunk c-001 L-c001-2: `monkeypatch.setattr(Path, "is_file", ...)` blast-radius claim — `pathlib.Path` and `album_builder.domain.library.Path` are the SAME class object (the module does `from pathlib import Path`), so a "narrower" patch target would have identical blast radius. The actual scope guard is the `self.name == "bad.mp3"` filter inside the patched function. Kept the original code with a clarifying comment.
- Chunk c-001 dead-code note on `tests/persistence/test_TC_10_atomic_pair.py` branch 2 — flagged as "untested" but the chunk agent didn't see the ANTS-equivalent `test_atomic_pair_scan_idempotent` covers the same code path with stale-tmp inputs. Even so, the explicit `test_TC_10_21_complete_pair_with_stale_tmps_sweeps_tmps` is more discriminating, so added anyway.

Calibration: 67 raw → ~40 actionable → ~35 closed inline + 5 deferred. Lower noise rate than the first sweep (40% vs 80% in the first run) because the first pass already absorbed the project-wide `# Spec:` debt; the residual findings are real but smaller.
...
- 🚧 **Distribution & cross-platform packaging (downloadable, runnable builds).** (User request 2026-07-25.) Turn Album Builder from a source-only, Linux-only checkout into downloadable "one file, install and run" builds on Linux and Windows, plus store/repo presence. **Supersedes the "Windows port (cross-platform packaging)" bullet above** (folded in). Spec-first (Spec 22 in progress) + `/cold-eyes` like every phase. Locked decisions (2026-07-25): sequence **Groundwork -> Flatpak/Flathub -> Windows -> OBS**; the heavy WhisperX/torch dependency stays an optional `pip` extra and is kept **out** of the bundles (keeps downloads to a few hundred MB, not GB). Phases:
  - **Phase Dist-1 — Portability groundwork (code).** Windows-safe export: replace `os.symlink` in `services/export.py` with a hardlink-then-copy fallback chain (the fallback the Spec 08 header comment already anticipates as "deferred to v0.6+"); swap direct `XDG_CONFIG_HOME` reads for Qt's `QStandardPaths`; swap the `xdg-open` folder-open for `QDesktopServices.openUrl`. New tests on a simulated no-symlink filesystem. Prerequisite for both Windows and Flatpak.
  - **Phase Dist-2 — Flatpak + Flathub.** Flatpak manifest bundling PyQt6 + WeasyPrint's Pango/Cairo/GDK-PixBuf runtime + GStreamer; an **AppStream metainfo** file (`packaging/` ships only a `.desktop` template today) + screenshots for the store; a single-file `.flatpak` bundle attached to GitHub Releases; and the Flathub submission PR. This is the best "one downloadable Linux file" answer and doubles as the app-store listing.
  - **Phase Dist-3 — Windows bundle.** PyInstaller one-folder / one-file `.exe` (bundling WeasyPrint's native libraries is the hard part; audio uses the built-in Windows backend, so no GStreamer needed there); attached to GitHub Releases. Note: unsigned builds trip Windows SmartScreen until code-signing is arranged.
  - **Phase Dist-4 — openSUSE Build Service (OBS).** Native RPM + DEB packages across distros via OBS for package-manager installs (zypper / apt) — distinct from the single-file Flatpak. Integrate with the OBS project a companion session ("finbreak") already set up for the user rather than starting from scratch.

- 📋 **Music player — standalone experience & popular-app feature parity (v2 epic; sequenced AFTER Distribution).** (User request 2026-07-25: "copy as many features as you can from Spotify / WinAmp / any popular music player.") Continues the shipped "Fully-featured music player mode" epic (Phases A-G) toward a player that stands on its own. **Locked direction (2026-07-25):** stay **one program** (not two executables), but grow the **Player tab into a self-sufficient player** — browse and start songs in place, with its own identity, so listening never requires the curation tab. **Reuse-before-rewrite** on `Player` / `PlaybackController` / `PlayQueue` / `PlaylistStore` / `Library` / `UsageIndex`; every phase is spec-first + `/cold-eyes`. *Already shipped, not re-listed here:* play queue, shuffle/repeat, saved playlists, MPRIS media-key + lock-screen integration, tray, synced lyrics, ReplayGain volume levelling, cover art, full transport, cross-album usage indicator. **Planned phases (rough value-first order):**
  ...
  - **Phase MP-2 — Player-tab standalone browsing.** A browsable library on the Player tab with double-click / Enter to play, "play from here", and enqueue — so listening never requires hopping to the curation tab. Delivers the locked "standalone player" direction (and fixes the "where's my library?" confusion a user hits when sitting on the Player tab).
  - **Phase MP-3 — Library browsing power.** Incremental search/filter box, multi-column sort (title / artist / album / duration / date-added / play-count), jump-to-letter, and optional group-by-artist/album (folds in the deferred "Group-by-artist tabs" bullet). (Spotify search; WinAmp / foobar2000 media library.)
  ...
  - **Phase MP-8 — Extras (survey; pick as desired).** Sleep timer, playback speed / pitch, A-B repeat, "play next" / queue history, drag-and-drop files or folders to enqueue, expanded global keyboard shortcuts, context-menu "go to album / artist", and optional Last.fm scrobbling. **Out of scope (carry-forward from the A-G epic):** streaming / Spotify integration, CD ripping, bulk tag editing, Discord presence.
